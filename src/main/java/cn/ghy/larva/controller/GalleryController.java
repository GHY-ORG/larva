...
public class GalleryController {
    ...
    @Autowired
    public GalleryController(IPostService iPostService, FileService fileService) {
    ...
    public Response upload(HttpServletRequest request) throws IOException {
        ...
        ArrayList<File> files = fileUtil.upload(request, "gallery", description);
        for (File file : files) {
            fileService.insert(file);

        }
        return new Response(200, files);
    }
}
